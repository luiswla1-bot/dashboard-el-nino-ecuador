...
SECCIONES = ["📈 Índice RONI", "🔮 Pronóstico", "🌦️ Clima en Vivo", "📍 Estaciones", "📊 Análisis"]
TITULOS_SECCION = {
    "📈 Índice RONI": "Índice Relativo Oceánico Niño (RONI) - NOAA/CPC",
    "🔮 Pronóstico": "Pronóstico Oficial NOAA/CPC — Próximas 9 Temporadas",
...
def obtener_probabilidades_enso():
    """Obtiene la tabla oficial de probabilidades de pronóstico ENSO (próximas 9 temporadas) de NOAA/CPC"""
    try:
        ...
        if response.status_code == 200:
            ...
            for tabla in tablas:
                ...
                if any('Niña' in c or 'Nina' in c for c in columnas):
                    tabla.columns = ['Temporada', 'La Niña', 'Neutral', 'El Niño']
                    for col in ['La Niña', 'Neutral', 'El Niño']:
                        tabla[col] = pd.to_numeric(tabla[col], errors='coerce')
                    return tabla.dropna()
    except Exception as e:
        st.caption(f"No se pudo obtener el pronóstico de probabilidades: {e}")
    return None
...
if seccion_actual == "📈 Índice RONI":
    ...
    if df_oni is not None and len(df_oni) > 0:
        ...
        valor_actual = ultimos_datos['oni'].iloc[-1]
        fecha_actual = df_oni['fecha'].iloc[-1]

        fig = go.Figure()

        # Zonas de fondo coloreadas para lectura inmediata (en vez de solo líneas punteadas)
        fig.add_hrect(y0=0.5, y1=df_oni['oni'].max() + 0.5, fillcolor="red", opacity=0.08, line_width=0)
        fig.add_hrect(y0=-0.5, y1=0.5, fillcolor="gray", opacity=0.06, line_width=0)
        fig.add_hrect(y0=df_oni['oni'].min() - 0.5, y1=-0.5, fillcolor="blue", opacity=0.08, line_width=0)

        # Línea principal con relleno hasta cero (evolución del RONI)
        fig.add_trace(go.Scatter(
            x=df_oni['fecha'], y=df_oni['oni'], mode='lines', name='RONI',
            line=dict(color='#2c5f8a', width=2), fill='tozeroy',
            fillcolor='rgba(44,95,138,0.15)'
        ))
        ...
        # Resaltar el punto/valor más reciente con una etiqueta directa
        fig.add_trace(go.Scatter(
            x=[fecha_actual], y=[valor_actual], mode='markers+text',
            marker=dict(size=12, color='gold', line=dict(color='black', width=1)),
            text=[f"  Actual: {valor_actual:.2f}°C"], textposition="middle right",
            showlegend=False, name='Actual'
        ))

        fig.update_layout(
            title="Evolución del Índice RONI", xaxis_title=None,
            yaxis_title="Anomalía (°C)", hovermode='x unified', height=430,
            showlegend=False,
            xaxis=dict(
                rangeslider=dict(visible=True, thickness=0.06),
                range=[df_oni['fecha'].max() - pd.Timedelta(days=365*6), df_oni['fecha'].max() + pd.Timedelta(days=90)]
            ),
            margin=dict(t=40, b=10)
        )
        st.plotly_chart(fig, use_container_width=True)
        st.caption("🔍 Usa la barra deslizante debajo del gráfico para explorar el histórico completo desde 1950")

        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("RONI Actual", f"{valor_actual:.2f}°C")
        with col2:
...
# ============ SECCIÓN: PRONÓSTICO ENSO ============
elif seccion_actual == "🔮 Pronóstico":
    st.caption("📌 Fuente: NOAA/CPC — consenso oficial de pronosticadores CPC/IRI, actualizado el 2° jueves de cada mes")

    df_prob = obtener_probabilidades_enso()
    if df_prob is not None and len(df_prob) > 0:
        fig_prob = go.Figure()
        fig_prob.add_trace(go.Bar(x=df_prob['Temporada'], y=df_prob['La Niña'], name='La Niña', marker_color='#1f77b4'))
        fig_prob.add_trace(go.Bar(x=df_prob['Temporada'], y=df_prob['Neutral'], name='Neutral', marker_color='#a0a0a0'))
        fig_prob.add_trace(go.Bar(x=df_prob['Temporada'], y=df_prob['El Niño'], name='El Niño', marker_color='#d62728'))
        fig_prob.update_layout(
            barmode='stack', height=480, yaxis_title="Probabilidad (%)",
            legend=dict(orientation='h', y=1.1), margin=dict(t=40, b=20),
            font=dict(size=13)
        )
        st.plotly_chart(fig_prob, use_container_width=True)
        st.caption("📌 0% = probabilidad prácticamente nula (no necesariamente cero exacto)")
    else:
        st.info("No se pudo cargar el pronóstico de probabilidades en este momento.")
...
# ============ SECCIÓN: ANÁLISIS + HISTÓRICO ============
    ...
    if len(hist) > 1:
        fig_hist = make_subplots(
            ...
            subplot_titles=("Índice RONI (Anomalía °C)", "Temperatura Superficial del Mar — Promedio (°C)")
        )

        # Panel superior: RONI con zonas de referencia
        fig_hist.add_hrect(y0=0.5, y1=max(2, hist['oni'].max() + 0.3), fillcolor="red", opacity=0.07, line_width=0, row=1, col=1)
        fig_hist.add_hrect(y0=-0.5, y1=0.5, fillcolor="gray", opacity=0.05, line_width=0, row=1, col=1)
        fig_hist.add_trace(
            go.Scatter(x=hist['timestamp'], y=hist['oni'], mode='lines+markers', name='RONI',
                       line=dict(color='#2c5f8a', width=2), marker=dict(size=5)),
            row=1, col=1
        )
        fig_hist.add_hline(y=0.5, line_dash="dash", line_color="red", opacity=0.5, row=1, col=1)
        fig_hist.add_hline(y=-0.5, line_dash="dash", line_color="blue", opacity=0.5, row=1, col=1)

        # Panel inferior: TSM
        fig_hist.add_trace(
            go.Scatter(x=hist['timestamp'], y=hist['tsm_promedio'], mode='lines+markers', name='TSM Promedio',
                       line=dict(color='#d68a00', width=2), marker=dict(size=5)),
            row=2, col=1
        )

        fig_hist.update_xaxes(tickformat="%d %b\n%H:%M", row=2, col=1)
        fig_hist.update_yaxes(title_text="°C", row=1, col=1)
        fig_hist.update_yaxes(title_text="°C", row=2, col=1)
        fig_hist.update_layout(height=430, showlegend=False, margin=dict(t=40, b=20))

        st.plotly_chart(fig_hist, use_container_width=True)
        st.caption(f"⏱️ Recolectando desde: {hist['timestamp'].iloc[0].strftime('%Y-%m-%d %H:%M')} — {len(hist)} lecturas guardadas en Google Sheets (1 cada ~30 min)")
