...
modo_tv = st.sidebar.toggle("📺 Pantalla completa (rotación automática)", value=False)
...
SECCIONES = ["📈 Índice RONI", "🌦️ Clima en Vivo", "🌡️ Temp. Marina", "📍 Estaciones", "📊 Análisis"]
...
<div style='display:flex; align-items:center; justify-content:space-between;
            flex-wrap:wrap; padding:6px 4px 10px 4px; border-bottom:1px solid rgba(128,128,128,0.3);
            margin-bottom:10px;'>
    <div style='font-size:1.4rem; font-weight:700;'>🌊 Monitoreo del Fenómeno del Niño</div>
    <div style='display:flex; gap:18px; font-size:0.8rem; opacity:0.85; flex-wrap:wrap;'>
        <span>🕐 {datetime.now().strftime('%H:%M:%S')}</span>
        <span>📍 {seccion_actual}</span>
        <span>{estado_tv}</span>
...
if seccion_actual == "📈 Índice RONI":
    st.subheader("Índice Relativo Oceánico Niño (RONI) - NOAA/CPC")
...
# ============ SECCIÓN: CLIMA EN VIVO ============
    ...
    st.subheader("Mapa de Clima en Tiempo Real")
    st.caption("🌦️ Fuente: Windy.com (modelo ECMWF/GFS) — cambia la capa (lluvia, viento, nubes, temperatura, olas) desde el menú del propio mapa")

    windy_html = """
    <div style="display:flex; justify-content:center; width:100%;">
        <iframe
            style="width:100%; max-width:1400px; height:78vh; min-height:500px; border:0;"
            src="https://embed.windy.com/embed2.html?lat=-1.5&lon=-80.0&detailLat=-1.5&detailLon=-80.0&zoom=6&level=surface&overlay=rain&product=ecmwf&menu=true&message=true&marker=&calendar=now&pressure=&type=map&location=coordinates&detail=&metricWind=default&metricTemp=default&radarRange=-1">
        </iframe>
    </div>
    """
    components.html(windy_html, height=650, scrolling=False)

# ============ SECCIÓN: TEMPERATURA MARINA ============
elif seccion_actual == "🌡️ Temp. Marina":
    st.subheader("Anomalía de Temperatura Superficial del Mar (TSM)")
    st.caption("📍 Datos referenciales — para el dato oficial verificado ve a la sección 'Estaciones' → boletines INOCAR")
...
# ============ SECCIÓN: ESTACIONES ============
    ...
    st.warning(
        "⚠️ **Nota sobre estos datos:** INOCAR no ofrece una API pública automatizable, "
        "así que los valores de esta tabla son **referenciales/ilustrativos**, no una lectura oficial en vivo. "
        "Para el dato **oficial y verificado**, consulta los enlaces de boletines abajo."
    )
