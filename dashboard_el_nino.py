...
# ============ ESTADO PERSISTENTE DE SESIÓN (histórico mientras el app esté activo) ============
if "historico" not in st.session_state:
    st.session_state["historico"] = pd.DataFrame(columns=["timestamp", "oni", "tsm_promedio"])
...
def registrar_historico(oni_actual, tsm_promedio):
    """Agrega una fila al histórico de la sesión (se reinicia si el servidor duerme)"""
    nueva_fila = pd.DataFrame([{
        "timestamp": datetime.now(),
        "oni": oni_actual,
        "tsm_promedio": tsm_promedio
    }])
    st.session_state["historico"] = pd.concat(
        [st.session_state["historico"], nueva_fila], ignore_index=True
    ).tail(500)  # límite para no crecer infinito
...
with col4:
    st.metric("Lecturas registradas (sesión)", len(st.session_state["historico"]))
...
# ============ SECCIÓN: ANÁLISIS + HISTÓRICO ============
    ...
    st.divider()
    st.markdown("### 📈 Histórico recolectado en esta sesión")
    ...
    if len(hist) > 1:
        ...
        st.caption(f"⏱️ Recolectando desde: {hist['timestamp'].iloc[0].strftime('%Y-%m-%d %H:%M')} — {len(hist)} lecturas")
    else:
        st.info("Aún no hay suficientes lecturas para graficar la tendencia. Vuelve en unos minutos.")
